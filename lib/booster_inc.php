...
';
			...
			for($i=0;$i<count($treffer[0]);$i++)
			{
				...
				#else $dir = rtrim(dirname($_SERVER['SCRIPT_FILENAME']).'/'.$this->css_stringbase,'/');
...
';
		...
		else
		{
			...
			if(1==1 && (!file_exists($cachefile) || $filestime > filemtime($cachefile)))
			{
	...
	public function css()
	{
		...
		for($i=0;$i<sizeof($sources);$i++)
		{
			...
			if($source != '')
			{
				...
				if((is_dir($source) || is_file($source)) && $cachefile != '' && file_exists($cachefile) && filemtime($cachefile) >= $filestime) $filescontent .= file_get_contents($cachefile);
				else
				{
